...
struct co *co_start(const char *name, void (*func)(void *), void *arg) {
  ...
  p->context.gprs[6] = (uint64_t)(&(p->stack[STACK_SIZE-40])); // TODO:不知为什么16就会报 printf 错误, 8 和 24就可以
  // 设定状态机的在被销毁时需要进入的函数
  *(uint64_t *)(&(p->stack[STACK_SIZE-32])) = (uint64_t)co_exit;
  // 设定状态机的函数入口, 存放到 rsp 指向的地方
  *(uint64_t *)(&(p->stack[STACK_SIZE-40])) = (uint64_t)func; // 8可以，16不行的原因很可能是少了一些类似 金丝雀guard 之类的东西
